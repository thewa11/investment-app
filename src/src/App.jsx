      ...
      if (getmidasResponse.ok) {
        ...
        while ((match = regex.exec(html)) !== null) {
          ...
          if (symbols.includes(symbol)) {
            prices[symbol] = { price: parseFloat(price), change: '0', source: 'GetMidas' };
          }
        }
      ...
      if (uzmanparaResponse.ok) {
        ...
        symbols.forEach(symbol => {
          ...
          if (match) {
            ...
            prices[symbol] = { price: parseFloat(price), change: '0', source: 'UzmanPara' };
          }
        });
      ...
      prev.map(stock => {
        if (prices[stock.symbol]) {
          ...
          return {
            ...
            currentAmount: stock.quantity * newPrice
          };
        }
        return stock;
      })
      ...
      prev.map(stock => {
        if (stock.id === stockId) {
          return {
            ...stock,
            currentPrice: price,
            currentAmount: stock.quantity * price
          };
        }
        return stock;
      })
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-2 rounded-lg text-white ${isRefreshing ? 'bg-yellow-600' : connectionStatus === 'connected' ? 'bg-green-600' : 'bg-red-600'}`}
            >
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className={`p-3 rounded-lg border-2 ${totals.gainLoss >= 0 ? 'bg-green-900 border-green-600' : 'bg-red-900 border-red-600'}`}>
                ...
                <p className={`text-xl font-bold ${totals.gainLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>₺{(totals.gainLoss / 1000).toFixed(0)}K</p>
              </div>
              <div className={`p-3 rounded-lg border-2 ${totals.gainLossPercent >= 0 ? 'bg-green-900 border-green-600' : 'bg-red-900 border-red-600'}`}>
                ...
                <p className={`text-xl font-bold ${totals.gainLossPercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>{totals.gainLossPercent}%</p>
              </div>
            </div>
