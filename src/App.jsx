...
import { 
  Plus, Trash2, X, TrendingUp, Lock, Zap, 
  Eye, EyeOff, LayoutDashboard, Briefcase, Bell
} from 'lucide-react';
...
export default function App() {
  ...
  const [portfolio, setPortfolio] = useState(() => {
    ...
    try {
      return decryptData(saved) || [
        { id: 1, symbol: 'FROTO', quantity: 10, buyPrice: 1000, currentPrice: 1050 },
        { id: 2, symbol: 'TUPRS', quantity: 50, buyPrice: 160, currentPrice: 175 }
      ];
    } catch (e) {
      return [];
    }
  ...
  const [newItem, setNewItem] = useState({ symbol: '', quantity: '', buyPrice: '', currentPrice: '' });
  ...
  // --- HESAPLAMALAR ---
  const stats = useMemo(() => {
    const totalInv = portfolio.reduce((a, b) => a + (Number(b.quantity) * Number(b.buyPrice)), 0);
    const totalCur = portfolio.reduce((a, b) => a + (Number(b.quantity) * Number(b.currentPrice)), 0);
  ...
  const handleAddStock = () => {
    ...
    setNewItem({ symbol: '', quantity: '', buyPrice: '', currentPrice: '' });
    setIsAddModalOpen(false);
  };

  const removeStock = (id) => {
    if(window.confirm("Bu hisseyi portföyden çıkarmak istediğine emin misin?")) {
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 text-white font-sans">
        <div className="w-full max-w-sm bg-slate-900/40 backdrop-blur-3xl border border-white/5 p-10 rounded-[3rem] text-center shadow-2xl">
          ...
          />
          <button onClick={login} className="w-full bg-blue-600 p-5 rounded-2xl font-black hover:bg-blue-500 transition-all shadow-lg shadow-blue-900/20">SİSTEMİ AÇ</button>
        </div>
      </div>
    );
  }
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-[#0a0c12] p-8 rounded-[2.5rem] border border-white/5 flex flex-col justify-between">
                ...
                <div className="h-1.5 w-full bg-white/5 rounded-full mt-4 overflow-hidden">
                    <div className="h-full bg-blue-500" style={{width: stats.perc > 0 ? '100%' : '30%'}}></div>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-4">
               ...
               <div className="bg-blue-500/5 border border-blue-500/10 p-5 rounded-3xl flex gap-4 items-center">
                  ...
                  <p className="text-xs font-bold text-blue-100/80 leading-relaxed">
                    Baytutumlu Analizi: Portföyündeki şirketlerin KAP bildirimleri ve X paylaşımları anlık taranıyor. Şirket bazlı haberler burada görünecek.
                  </p>
               </div>
            </section>
          </>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center px-2">
              ...
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-2xl flex items-center gap-2 text-xs font-black transition-all shadow-lg"
              >
            ...
            <div className="grid gap-4">
                ...
                <div key={s.id} className="bg-[#0a0c12] p-5 rounded-[2rem] border border-white/5 flex justify-between items-center group transition-all hover:border-white/10">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className={`text-[10px] font-black ${Number(s.currentPrice) >= Number(s.buyPrice) ? 'text-emerald-500' : 'text-rose-500'}`}>
                        %{(((Number(s.currentPrice) - Number(s.buyPrice)) / Number(s.buyPrice)) * 100).toFixed(1)}
                      </p>
                    </div>
                    <button onClick={() => removeStock(s.id)} className="p-2 text-slate-700 hover:text-rose-500 transition-colors">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
              ...
              {portfolio.length === 0 && (
                <div className="text-center py-20 bg-white/5 rounded-[3rem] border border-dashed border-white/10">
                  <p className="text-slate-500 font-bold italic uppercase tracking-widest text-xs">Henüz hisse eklenmedi</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-[#0f172a] w-full max-w-md p-8 rounded-[3rem] border border-white/10 shadow-2xl space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-black text-white italic uppercase">Portföye Ekle</h3>
              <button onClick={() => setIsAddModalOpen(false)} className="text-slate-500"><X /></button>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <input placeholder="Hisse (Örn: THYAO)" className="bg-black/40 border border-white/5 p-4 rounded-2xl col-span-2 text-white font-bold outline-none focus:border-blue-500 uppercase" onChange={e => setNewItem({...newItem, symbol: e.target.value.toUpperCase()})} />
              <input type="number" placeholder="Adet" className="bg-black/40 border border-white/5 p-4 rounded-2xl text-white font-bold outline-none focus:border-blue-500" onChange={e => setNewItem({...newItem, quantity: e.target.value})} />
              <input type="number" placeholder="Maliyet" className="bg-black/40 border border-white/5 p-4 rounded-2xl text-white font-bold outline-none focus:border-blue-500" onChange={e => setNewItem({...newItem, buyPrice: e.target.value})} />
              <input type="number" placeholder="Güncel Fiyat" className="bg-black/40 border border-white/5 p-4 rounded-2xl col-span-2 text-white font-bold outline-none focus:border-blue-500" onChange={e => setNewItem({...newItem, currentPrice: e.target.value})} />
            </div>
            <button onClick={handleAddStock} className="w-full bg-blue-600 p-5 rounded-2xl font-black text-white shadow-xl hover:bg-blue-500 active:scale-95 transition-all">HİSSEYİ KAYDET</button>
          </div>
        </div>
      )}
